...
export default function PersonEdit({ isOpen, onClose, refresh, group }) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await $api.post(`/api/groups/${group.id}`, formData, {
        headers: {
      ...
      Swal.fire({
        title: "Xato!",
        text: error.response?.data?.message || "Guruh tahrirlashda xatolik yuz berdi.",
        icon: "error",
        position: "top-end",
        timer: 3000,
        timerProgressBar: true,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <div className="flex items-center justify-between">
          <h1 className="text-[25px] font-bold">Guruh tahrirlash</h1>
